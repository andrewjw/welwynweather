    ...
    if (file.endsWith(".json") && file.split(".")[0].match(/\d+/)) {
      fs.readdirSync("public/data/" + file.split(".")[0]).forEach(mfile => {
        if (mfile.endsWith(".json") && mfile.split(".")[0].match(/\d+/)) {
          paths.push({ params: { year: file.split(".")[0], month: mfile.split(".")[0] }});
        }
      });
    }
  });
...
export async function getStaticProps(context: any) {
  let raw = fs.readFileSync("public/data/" + context.params.year + "/" + context.params.month + ".json");

  return {
    props: { year: context.params.year, month: context.params.month, data: JSON.parse(raw.toString()) },
  }
}
