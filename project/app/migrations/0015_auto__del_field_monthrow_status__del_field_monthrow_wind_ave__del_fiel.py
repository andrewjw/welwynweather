...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'MonthRow.status'
        db.delete_column('app_monthrow', 'status')

        # Deleting field 'MonthRow.wind_ave'
        db.delete_column('app_monthrow', 'wind_ave')

        # Deleting field 'MonthRow.abs_pressure'
        db.delete_column('app_monthrow', 'abs_pressure')

        # Deleting field 'MonthRow.rained'
        db.delete_column('app_monthrow', 'rained')


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'MonthRow.status'
        raise RuntimeError("Cannot reverse this migration. 'MonthRow.status' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'MonthRow.wind_ave'
        raise RuntimeError("Cannot reverse this migration. 'MonthRow.wind_ave' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'MonthRow.abs_pressure'
        raise RuntimeError("Cannot reverse this migration. 'MonthRow.abs_pressure' and its values cannot be restored.")
        # Adding field 'MonthRow.rained'
        db.add_column('app_monthrow', 'rained',
